...
void main()
{
	iResolution = textureSize(target, 0).xy;
	...
	float coordX = fTexCoord.x / iResolution.x;
	float coordY = fTexCoord.y / iResolution.x;
	vec2 dstCoord = vec2(coordX, coordY);
	vec2 srcCoord = vec2(coordX, coordY / ratio);	
	vec2 uv = srcCoord.xy;

	uv = fTexCoord * iResolution;

	vec3 res = vec3(1.0, 1.0, 1.0);
    vec4 tex = texture(target, fTexCoord);
    ...
    {
		if (mod(uv.x + uv.y, density) <= width)
		{
#ifdef COLOUR_HATCHES
    ...
    {
		if (mod(uv.x + uv.y - (density*0.5), density) <= width)
		{
#ifdef COLOUR_HATCHES
